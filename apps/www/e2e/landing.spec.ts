...
test.describe('Demo Pages External Links', () => {
  ...
  test('Quartz page has install command, GitHub and Live Demo links', async ({ page }) => {
    ...
    await expect(page.getByText('npm install @quartz-headless/primitives')).toBeVisible();
...
test.describe('AppShell integration spike', () => {
  test('composes Quartz, Volt, Lumen and Movement on desktop', async ({ page }) => {
    await page.goto('/stack-shell');

    await expect(page.getByTestId('stack-shell-spike')).toBeVisible();
    await expect(page.getByRole('heading', { name: 'AppShell foundation' })).toBeVisible();
    await expect(page.getByText('Quartz controls sidebar and navbar state')).toBeVisible();

    await page.getByTestId('stack-shell-sidebar-trigger').click();
    await expect(page.getByTestId('stack-shell-sidebar-state')).toHaveText('expanded');

    await page.getByRole('button', { name: 'Collapse' }).click();
    await expect(page.getByTestId('stack-shell-sidebar-state')).toHaveText('collapsed');
  });

  test('supports mobile overlay trigger and dismissal', async ({ page }) => {
    await page.setViewportSize({ width: 390, height: 844 });
    await page.goto('/stack-shell');

    await page.getByTestId('stack-shell-sidebar-trigger').click();
    await expect(page.getByText('Andersseen App')).toBeVisible();

    await page.keyboard.press('Escape');
    await expect(page.getByTestId('stack-shell-sidebar-state')).toHaveText('closed');
  });
});
