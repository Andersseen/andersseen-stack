...
|   `-- create-andersseen-app/   # pnpm create andersseen-app (Minimal, Dashboard, Landing, Landing + Dashboard)
|-- docs/
|-- pnpm-workspace.yaml
`-- turbo.json
```
...
- Create: `pnpm create andersseen-app`, generates a new Analog application. Currently offers four shapes — `Minimal` (single-page starter), `Dashboard` (AppShell with sidebar, navbar and routed pages), `Landing` (public shell with navbar, hero, content section and footer) and `Landing + Dashboard` (the public site at `/` plus the routed Dashboard AppShell under `/dashboard`, composed from the same two shells rather than a duplicated template).
